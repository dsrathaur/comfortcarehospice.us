...
class Mail
{
	public static function Send($template, $subject, $templateVars, $to, $toName = null, $from = null, $fromName = null, $fileAttachment = null)
	{
			...
			if (!empty($fileAttachment) && is_array($fileAttachment)){
				foreach($fileAttachment as $attach){
					if(isset($attach['content']) && isset($attach['name'])){
						$message->attach(new Swift_Message_Attachment($attach['content'], $attach['name']));
					}
				}				
			}				
