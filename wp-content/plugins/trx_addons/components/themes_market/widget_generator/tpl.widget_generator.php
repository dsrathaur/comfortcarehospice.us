				...
				<div class="widget_generator_banner">
					<img src="<?php
							if (has_post_thumbnail()) {
								$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), trx_addons_get_thumb_size('masonry-big') );
								echo esc_url($image[0]);
							} else
								echo esc_url(trx_addons_get_file_url(TRX_ADDONS_PLUGIN_THEMES_MARKET.'widget_generator/widget_generator.png'));
							?>" alt="<?php esc_attr_e('ThemeREX Widget Generator', 'trx_addons'); ?>">
				</div>
			</div><?php
			// Description
