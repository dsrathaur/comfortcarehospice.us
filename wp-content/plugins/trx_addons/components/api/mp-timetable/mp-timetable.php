...
if ( !function_exists( 'trx_addons_mptt_load_scripts_front' ) ) {
	...
	function trx_addons_mptt_load_scripts_front() {
		if (trx_addons_exists_mptt() && trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-mp_timetable', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_API . 'mp-timetable/mp-timetable.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_mptt_merge_styles' ) ) {
	add_action("trx_addons_filter_merge_styles", 'trx_addons_mptt_merge_styles');
...
if ( !function_exists( 'trx_addons_mptt_importer_export' ) ) {
	...
	function trx_addons_mptt_importer_export($importer) {
		if ( trx_addons_exists_mptt() && in_array('mp-timetable', $importer->options['required_plugins']) ) {
			trx_addons_fpc(trx_addons_get_file_dir(TRX_ADDONS_PLUGIN_IMPORTER . 'export/mp-timetable.txt'), serialize( array(
