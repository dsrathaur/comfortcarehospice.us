	...
	function trx_addons_get_list_terms($prepend_inherit=false, $taxonomy='category', $opt=array()) {
		...
		if (empty($list[$hash])) {
			...
			if (!is_wp_error( $terms ) && is_array($terms) && count($terms) > 0) {
				$list[$hash] = trx_addons_get_hierarchical_list($terms);
			}
		}
		return $prepend_inherit 
...
if ( !function_exists( 'trx_addons_get_hierarchical_list' ) ) {
	function trx_addons_get_hierarchical_list($terms, $parent=0, $level=0) {
		$list = array();
		foreach ($terms as $term) {
			if ($term->parent == $parent ) {
				$list[$term->term_id] = ($level ? str_repeat('-', $level).' ': '')
										. $term->name . (!empty($term->count) ? ' (' . intval($term->count) . ')': '');
				$list = trx_addons_array_merge($list, trx_addons_get_hierarchical_list($terms, $term->term_id, $level+1));
			}
		}
		return $list;
	}
