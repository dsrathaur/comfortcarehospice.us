...
?><article id="post-<?php the_ID(); ?>" 
	<?php post_class( 'post_item post_layout_excerpt post_format_'.esc_attr($prodent_post_format) ); ?>
	...
	// Featured image
	prodent_show_post_featured(array( 'thumb_size' => prodent_get_thumb_size( strpos(prodent_get_theme_option('body_style'), 'full')!==false ? 'full' : 'big' ) ));
	...
	if (get_the_title() != '') {
		...
		<div class="post_header entry-header">
			...
			$prodent_components = prodent_is_inherit(prodent_get_theme_option_from_meta('meta_parts')) 
										? 'categories,date,author,counters'
	...
	?><div class="post_content entry-content"><?php
		if (prodent_get_theme_option('blog_content') == 'fullpost') {
			// Post content area
