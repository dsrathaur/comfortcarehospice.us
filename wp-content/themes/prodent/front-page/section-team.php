<div class="front_page_section front_page_section_team<?php
			$prodent_scheme = prodent_get_theme_option('front_page_team_scheme');
			if (!prodent_is_inherit($prodent_scheme)) echo ' scheme_'.esc_attr($prodent_scheme);
			echo ' front_page_section_paddings_'.esc_attr(prodent_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$prodent_css = '';
		$prodent_bg_image = prodent_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$prodent_anchor_icon = prodent_get_theme_option('front_page_team_anchor_icon');	
	$prodent_anchor_text = prodent_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (prodent_get_theme_option('front_page_team_fullheight'))
				echo ' prodent-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$prodent_css = '';
			$prodent_bg_mask = prodent_get_theme_option('front_page_team_bg_mask');
			$prodent_bg_color = prodent_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$prodent_caption = prodent_get_theme_option('front_page_team_caption');
			if (!empty($prodent_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($prodent_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($prodent_caption); ?></h2><?php
			}
		
			// Description (text)
			$prodent_description = prodent_get_theme_option('front_page_team_description');
			if (!empty($prodent_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($prodent_description) ? 'filled' : 'empty'; ?>"><?php echo wpautop(wp_kses_post($prodent_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!prodent_exists_trx_addons())
						...
						prodent_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
